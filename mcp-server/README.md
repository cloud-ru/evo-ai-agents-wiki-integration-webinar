...
- **Streamable HTTP** (`/`): Современный транспорт для новых клиентов
- **SSE** (`/sse` + `/messages`): Устаревший транспорт для обратной совместимости
...
Сервер поддерживает как современные, так и устаревшие MCP клиенты:

### Современные клиенты (Streamable HTTP)

Используйте основной эндпоинт `/` для подключения к серверу.

### Устаревшие клиенты (SSE)

Для клиентов, использующих устаревший SSE транспорт:

1. Подключитесь к эндпоинту `/sse`
2. Используйте `/messages` для отправки MCP сообщений
3. Сервер автоматически определит тип транспорта и обработает запрос
